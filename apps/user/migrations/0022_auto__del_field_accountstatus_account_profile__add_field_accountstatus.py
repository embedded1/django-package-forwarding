...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'AccountStatus.account_profile'
        db.delete_column('user_accountstatus', 'account_profile_id')

        # Adding field 'AccountStatus.profile'
        db.add_column('user_accountstatus', 'profile',
                      self.gf('django.db.models.fields.related.OneToOneField')(default=1, related_name='account_status', unique=True, to=orm['user.Profile']),
                      keep_default=False)


    def backwards(self, orm):

        # User chose to not deal with backwards NULL issues for 'AccountStatus.account_profile'
        raise RuntimeError("Cannot reverse this migration. 'AccountStatus.account_profile' and its values cannot be restored.")
        
        # The following code is provided here to aid in writing a correct migration        # Adding field 'AccountStatus.account_profile'
        db.add_column('user_accountstatus', 'account_profile',
                      self.gf('django.db.models.fields.related.OneToOneField')(related_name='account_status', unique=True, to=orm['user.Profile']),
                      keep_default=False)

        # Deleting field 'AccountStatus.profile'
        db.delete_column('user_accountstatus', 'profile_id')
