...
from paypal.adaptive.exceptions import (
    ...
    GeneralException)
from .exceptions import BitcoinPayError
...
class RedirectView(PaymentSourceMixin,
    ...
    def get_redirect_url(self, **kwargs):
        ...
        except BitcoinPayError, e:
            logger.error("BitcoinPay error: %s" % e)
            messages.error(
    ...
    def _get_redirect_url(self, **kwargs):
        ...
        if not redirect_url:
            raise BitcoinPayError("Empty redirect url")
    ...
    def handle_payment(self, order_number, total, **kwargs):
        ...
        partner_share, _ = self.get_partner_payment_info(self.basket, payment_processor=settings.BITCOIN_PAY_LABEL)
        source_type, _ = SourceType.objects.get_or_create(name=settings.BITCOIN_PAY_LABEL)
...
class SuccessResponseView(CheckoutSessionMixin, generic.RedirectView):
    ...
    def cancel_order(self):
        ...
        if basket_id:
            try:
                ...
                messages.error(self.request, _("No order found"))
            else:
                basket.thaw()
    ...
    def get(self, request, *args, **kwargs):
        ...
        status = request.GET.get('bitcoinpay-status', '')
        if status in ('cancel', 'false'):
            self.cancel_order()
            if not self.order_cancelled:
